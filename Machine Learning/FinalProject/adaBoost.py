...
scores = cross_validation.cross_val_score(boost, train_x, train_y, cv = 5, scoring = 'accuracy')
...
(pd.concat([test, result], axis = 1)).to_csv(sys.argv[2].split('/input/')[0] + '/output/AB_' + sys.argv[3].split('/input/')[1].split('.')[0] + '.csv', index = False, float_format = '%g')
